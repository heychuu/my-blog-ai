...
# 헤이츄 스타일 가이드 (강조 기호 금지 및 분량 엄격 조절)
HYEJU_STYLE = """
...
def generate_post(images, context, api_key):
    # 최신 v1.x 라이브러리 방식으로 클라이언트 선언
    client = genai.Client(api_key=api_key)
    
    prompt = f"{HYEJU_STYLE}\n\n[사용자 메모]: {context}\n\n사진을 분석해서 1,700자 내외로 작성해줘."
    
    # 텍스트 힌트와 이미지들을 하나의 리스트로 결합
    contents = [prompt] + images
    
    # 멀티모달 처리에 가장 안정적인 gemini-2.5-flash 모델 적용
    ...
    if not api_key:
        ...
        with st.spinner("작성 중..."):
            try:
                ...
                result = generate_post(images, memo, api_key)
                st.subheader("📝 완성된 초안")
